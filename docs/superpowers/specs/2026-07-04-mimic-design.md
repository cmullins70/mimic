...
6. **SSH transport supply chain (audited 2026-07-04):** Citadel has always run on a maintainer fork of `apple/swift-nio-ssh` that diverged from Apple upstream in May 2022 (~83 commits behind Apple's ongoing hardening). Citadel 0.12.1 additionally swapped that dependency to an unvetted third-party fork (`Wellz26/swift-nio-ssh`) via an unreviewed PR. Mitigation in place: Citadel pinned `exact: 0.12.0` (last release on the maintainer's own fork); the fork's independently rewritten version parser was hand-verified immune to upstream's known bare-LF DoS bug. Before any public release: vendor our own `swift-nio-ssh` rebased from Apple upstream (cherry-picking Citadel's protocol extensions), or reassess the SSH library. Any Citadel version bump requires reviewing its dependency URLs first.
